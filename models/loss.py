...
def criterion_GAN(output, target, use_lsgan=False, use_wesserstein=True):
    assert np.sum(
        [use_lsgan, use_wesserstein]) < 2, "['use_lsgan', 'use_wesserstein'] parameters!"
...
def generator_loss(G_tensors, cycle_loss_weight=.3, id_loss_weight=.3, use_wgan=True):
...
def discriminator_loss(netD_predict, use_wgan=True):
