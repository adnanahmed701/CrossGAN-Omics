...
def create_batch_generators(data_path, train_file, test_file, input_shape, batch_size):
    ...
    print(f"DEBUG: train_A shape: {train_A.shape}, label_A shape: {label_A.shape}")
    print(f"DEBUG: train_B shape: {train_B.shape}, label_B shape: {label_B.shape}")
...
def process_test_data(generators_tuple, test_data_tuple, save_path,
    ...
    dfA.to_csv(os.path.join(save_path, 'outdataB_test1(0.5, 0.3).csv'), index=False)
    ...
    dfB.to_csv(os.path.join(save_path, 'outdataA_test1(0.5, 0.3).csv'), index=False)
